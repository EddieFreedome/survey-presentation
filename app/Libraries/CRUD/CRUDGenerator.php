...
class CRUDGenerator
{
    ...
    {
        if (is_dir(app_path('Http/Controllers/Admin/' . ucfirst($this->name) . 'Controller.php'))) {
            
            // dd('if', is_dir(app_path('Http/Controllers/Admin/' . ucfirst($this->name) . 'Controller.php')));
            $this->generateFile(
            ...
            //it is nececessary to specify permission and -> TRUE!
            //specifying true it will create subdirectories too
            mkdir($path.'Admin', 0777, true);
            $this->generateFile(
                app_path('Http/Controllers/Admin/' . ucfirst($this->name) . 'Controller.php'),
                app_path('Libraries/CRUD/templates/app/Http/Controllers/Admin/CRUD_filenameController.php')
            );
